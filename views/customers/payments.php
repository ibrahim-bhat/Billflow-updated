...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (isset($_POST['edit_payment'])) {
            ...
            // Update customer balance: First reverse the old payment, then apply the new payment
            ...
            $sql = "SELECT * FROM customer_payments WHERE id = ?";
            $stmt = $conn->prepare($sql);
            ...
            $sql = "UPDATE customers SET balance = balance + ? WHERE id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("di", $total_payment, $customer_id);
            $stmt->execute();
        ...
        $conn->commit();
        header("Location: payments.php?id=" . $customer_id);
        exit();
...
<div class="main-content">
    <div class="page-header d-flex justify-content-between align-items-center">
        <div>
            ...
            <p>Edit and manage payment entries for <?php echo htmlspecialchars($customer['name']); ?></p>
        </div>
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-3">
                    <strong>Name:</strong> <?php echo htmlspecialchars($customer['name']); ?>
                </div>
                <div class="col-md-3">
                    <strong>Contact:</strong> <?php echo htmlspecialchars($customer['contact']); ?>
                </div>
                <div class="col-md-3">
                    <strong>Location:</strong> <?php echo htmlspecialchars($customer['location']); ?>
                </div>
                <div class="col-md-3">
                    <strong>Current Balance:</strong> 
                    <span class="badge <?php echo $customer['balance'] > 0 ? 'bg-danger' : 'bg-success'; ?>">
                        ₹<?php echo number_format(abs($customer['balance']), 2); ?>
                        <?php echo $customer['balance'] > 0 ? ' (Receivable)' : ' (Payable)'; ?>
                    </span>
                </div>
            </div>
        </div>
            ...
            <form method="post">
                <div class="modal-body">
                    ...
                    <div class="alert alert-warning">
                        <strong>Warning!</strong> This action will permanently delete the payment and adjust the customer balance accordingly.
                    </div>
